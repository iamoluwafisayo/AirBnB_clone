...
"""
    ...
    def test_do_create_error(self):
        """Tests generate command with errors."""
        with patch('sys.stdout', new=StringIO()) as f:
    ...
    def test_do_show(self):
        """Tests show for all the  classes."""
        for classname in self.classes():
    ...
    def create_class(self, classname):
        """Generates a class for console tests."""
        with patch('sys.stdout', new=StringIO()) as f:
            HBNBCommand().onecmd("create {}".format(classname))
        uid = f.getvalue()[:-1]
        self.assertTrue(len(uid) > 0)
    ...
    def help_load_dict(self, rep):
        """Helper method to test dict equality."""
        rex = re.compile(r"^\[(.*)\] \((.*)\) (.*)$")
        res = rex.match(rep)
